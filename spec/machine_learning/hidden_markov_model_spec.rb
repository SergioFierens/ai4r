...
RSpec.describe Ai4r::MachineLearning::HiddenMarkovModel do
  # Test data
  let(:weather_states) { %i[sunny rainy] }
  let(:activity_observations) { %i[walk shop clean] }

  let(:simple_sequences) do
    [
      { observations: %i[walk shop clean], states: %i[sunny sunny rainy] },
      { observations: %i[clean clean walk], states: %i[rainy rainy sunny] },
      { observations: %i[walk walk shop], states: %i[sunny sunny sunny] }
    ]
  end
  ...
  let(:unsupervised_sequences) do
    [
      ...
      { observations: %i[clean clean walk] },
      { observations: %i[walk walk shop] },
      { observations: %i[shop clean clean] },
      { observations: %i[walk clean shop] }
    ]
  end

  let(:binary_states) { %i[on off] }
  let(:binary_observations) { %i[beep silence] }
  let(:binary_sequences) do
    [
      { observations: %i[beep beep silence], states: %i[on on off] },
      { observations: %i[silence silence beep], states: %i[off off on] },
      { observations: %i[beep silence beep], states: %i[on off on] }
    ]
  end
  ...
  describe 'initialization' do
    ...
    context 'with invalid parameters' do
      it 'raises error for empty states' do
        ...
        end.to raise_error(ArgumentError, 'States cannot be empty')
      end

      it 'raises error for empty observations' do
        expect do
          described_class.new(weather_states, [])
        end.to raise_error(ArgumentError, 'Observations cannot be empty')
      end

      it 'raises error for duplicate states' do
        expect do
          described_class.new(%i[sunny sunny], activity_observations)
        end.to raise_error(ArgumentError, 'States must be unique')
      end

      it 'raises error for duplicate observations' do
        expect do
          described_class.new(weather_states, %i[walk walk])
        end.to raise_error(ArgumentError, 'Observations must be unique')
      end
    end
  end
  ...
  describe 'supervised training' do
    ...
    context 'with invalid sequences' do
      it 'raises error for empty sequences' do
        ...
        end.to raise_error(ArgumentError, 'Training sequences cannot be empty')
      end
      ...
      it 'raises error for unknown observations' do
        ...
        end.to raise_error(ArgumentError, 'Unknown observation: unknown')
      end
      ...
      it 'raises error for unknown states' do
        ...
        expect do
          hmm.train(invalid_sequence, supervised: true)
        end.to raise_error(ArgumentError, 'Unknown state: unknown')
      end

      it 'raises error for mismatched sequence lengths' do
        invalid_sequence = [{ observations: %i[walk shop], states: [:sunny] }]
        expect do
          hmm.train(invalid_sequence, supervised: true)
        end.to raise_error(ArgumentError, 'Sequence 0 states and observations must have same length')
      end
    end
  end
  ...
  describe 'unsupervised training' do
    ...
    context 'with valid sequences' do
      ...
      it 'improves log-likelihood over iterations' do
        ...
        expect(hmm.log_likelihood.last).to be >= hmm.log_likelihood.first if hmm.log_likelihood.length > 1
      end
    ...
    context 'with limited data' do
      it 'handles single sequence' do
        single_sequence = [{ observations: %i[walk shop clean] }]
  ...
  describe 'forward algorithm' do
    ...
    context 'with valid sequences' do
      ...
      it 'produces consistent results' do
        prob1 = hmm.forward(%i[walk shop])
        prob2 = hmm.forward(%i[walk shop])

        expect(prob1).to eq(prob2)
      end

      it 'handles longer sequences' do
        long_sequence = %i[walk shop clean walk shop]
    ...
    context 'with invalid sequences' do
      it 'raises error for empty sequence' do
        expect do
          hmm.forward([])
        end.to raise_error(ArgumentError, 'Observation sequence cannot be empty')
      end
  ...
  describe 'viterbi algorithm' do
    ...
    context 'with valid sequences' do
      it 'finds most likely state sequence' do
        states = hmm.viterbi(%i[walk shop])
      ...
      it 'produces consistent results' do
        states1 = hmm.viterbi(%i[walk shop])
        states2 = hmm.viterbi(%i[walk shop])
  ...
  describe 'sequence generation' do
    ...
    context 'with invalid parameters' do
      it 'raises error for untrained model' do
        ...
        end.to raise_error(RuntimeError, 'Model must be trained before generating sequences')
      end
    end
  end
  ...
  describe 'model evaluation' do
    ...
    context 'with valid test sequences' do
      it 'evaluates model performance' do
        test_sequences = [
          { observations: %i[walk shop] },
          { observations: %i[clean clean] }
        ]
      ...
      it 'calculates state accuracy when states provided' do
        test_sequences = [
          { observations: %i[walk shop], states: %i[sunny sunny] },
          { observations: %i[clean clean], states: %i[rainy rainy] }
        ]
  ...
  describe 'model analysis' do
    ...
    context 'with untrained model' do
      it 'raises error for untrained model' do
        ...
        end.to raise_error(RuntimeError, 'Model must be trained before analysis')
      end
    end
  end
  ...
  describe 'model visualization' do
    ...
    context 'with untrained model' do
      it 'raises error for untrained model' do
        ...
        end.to raise_error(RuntimeError, 'Model must be trained before visualization')
      end
    end
  end
  ...
  describe 'educational features' do
    ...
    it 'provides verbose output during forward algorithm' do
      hmm.train(simple_sequences, supervised: true)
      expect { hmm.forward(%i[walk shop]) }.to output(/Forward Algorithm/).to_stdout
    end

    it 'provides verbose output during viterbi algorithm' do
      hmm.train(simple_sequences, supervised: true)
      expect { hmm.viterbi(%i[walk shop]) }.to output(/Viterbi Algorithm/).to_stdout
    end
  end
    ...
    context 'boundary conditions' do
      ...
      it 'handles single state model' do
        ...
        single_state_sequences = [
          { observations: %i[walk shop], states: %i[only_state only_state] }
        ]
      ...
      it 'handles single observation model' do
        ...
        single_obs_sequences = [
          { observations: %i[only_obs only_obs], states: %i[sunny rainy] }
        ]
