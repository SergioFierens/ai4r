...
RSpec.describe Ai4r::Data::Proximity do
  ...
  let(:categorical_a) { %w[red large circle] }
  let(:categorical_b) { %w[blue small square] }
  let(:categorical_c) { %w[red large circle] }  # Identical to categorical_a
  let(:categorical_d) { %w[red small circle] }  # Partially similar
