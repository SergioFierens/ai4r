...
RSpec.describe 'Modern Outlier Detection' do
  ...
  let(:sample_labels) { %w[feature1 feature2 feature3] }
  ...
  describe Ai4r::Data::ModernOutlierDetector do
    ...
    describe '#detect' do
      ...
      context 'error handling' do
        it 'raises error for unknown method' do
          ...
          end.to raise_error(Ai4r::Data::OutlierDetectionError) do |error|
    ...
    describe '#comprehensive_analysis' do
      it 'runs multiple detection methods' do
        analysis = detector.comprehensive_analysis(methods: %i[iqr z_score])
        ...
        expect(analysis.overall_severity).to be_in(%i[low moderate high severe])
      end

      it 'finds consensus outliers' do
        analysis = detector.comprehensive_analysis(methods: %i[iqr z_score modified_z_score])
    ...
    describe 'educational features' do
      ...
      it 'provides educational explanations' do
        ...
        end.to output(/💡 IQR Method Explanation/).to_stdout
      end

      it 'shows detection results' do
        expect do
          educational_detector.detect(method: :iqr)
        end.to output(/📊 Outlier Detection Results/).to_stdout
      end

      it 'tracks progress during comprehensive analysis' do
        expect do
          educational_detector.comprehensive_analysis(methods: %i[iqr z_score])
        end.to output(/📊 Learning Progress/).to_stdout
      end
    end
    ...
    describe 'performance characteristics' do
      ...
      it 'performs comprehensive analysis efficiently' do
        benchmark_performance('Comprehensive analysis') do
          large_detector.comprehensive_analysis(methods: %i[iqr z_score])
        end
      end
    end
  ...
  describe 'ComprehensiveAnalysisResult' do
    ...
    let(:comprehensive_result) do
      Ai4r::Data::ModernOutlierDetector::ComprehensiveAnalysisResult.new(
        ...
        consensus_outliers: { 'feature1' => { 5 => %i[iqr z_score] } },
