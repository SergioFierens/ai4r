...
RSpec.describe Ai4r::Classifiers::ZeroR do
  ...
  let(:majority_class_dataset) do
    Ai4r::Data::DataSet.new(
      data_items: [
        %w[A B yes], %w[C D yes], %w[E F yes],
        %w[G H yes], %w[I J yes], %w[K L no],
        %w[M N no], %w[O P maybe]
      ],
  ...
  let(:tie_dataset) do
    Ai4r::Data::DataSet.new(
      data_items: [%w[A B class1], %w[C D class1], %w[E F class2], %w[G H class2]],
      data_labels: %w[attr1 attr2 class]
    )
  end
  ...
  let(:single_class_dataset) do
    Ai4r::Data::DataSet.new(
      data_items: [%w[A B only_class], %w[C D only_class], %w[E F only_class]],
      data_labels: %w[attr1 attr2 class]
    )
  end
  ...
  let(:many_classes_dataset) do
    ...
    Ai4r::Data::DataSet.new(
      data_items: items,
      data_labels: %w[num_attr mult_attr class]
    )
  end
  ...
  describe 'Build Tests' do
    context 'majority class identification' do
      it 'test_build_majority_class' do
        ...
        result = classifier.eval(%w[anything irrelevant])
  ...
  describe 'Eval Tests' do
    ...
    context 'baseline behavior' do
      it 'test_eval_always_same' do
        ...
        result1 = trained_classifier.eval(%w[sunny hot])
        result2 = trained_classifier.eval(%w[rainy cold])
        result3 = trained_classifier.eval(%w[overcast mild])
      ...
      it 'test_eval_ignores_features' do
        ...
        result2 = trained_classifier.eval(%w[A B C D E])
  ...
  describe 'Performance Tests' do
    ...
    it 'handles massive datasets efficiently' do
      ...
      massive_items = Array.new(10_000) { |i| [i, i * 2, i * 3, i.even? ? 'even' : 'odd'] }
  ...
  describe 'Statistical Properties Tests' do
    it 'correctly identifies majority in various distributions' do
      ...
      uniform_items = Array.new(35) do |i|
        [rand(10), if i < 10
                     'A'
                   else
                     (i < 20 ? 'B' : 'C')
      ...
      uniform_dataset = Ai4r::Data::DataSet.new(
        ...
        data_labels: %w[random_attr class]
      )
