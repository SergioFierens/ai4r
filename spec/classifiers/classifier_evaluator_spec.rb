...
RSpec.describe Ai4r::Classifiers::ClassifierEvaluator do
  ...
  let(:perfect_predictions) { ['A', 'B', 'A', 'C', 'B', 'A', 'C', 'B'] }
  let(:perfect_actual) { ['A', 'B', 'A', 'C', 'B', 'A', 'C', 'B'] }
  
  let(:mixed_predictions) { ['A', 'B', 'A', 'C', 'A', 'A', 'C', 'C'] }
  let(:mixed_actual) { ['A', 'B', 'A', 'C', 'B', 'A', 'C', 'B'] }
  
  let(:binary_predictions) { ['yes', 'no', 'yes', 'no', 'yes', 'no'] }
  let(:binary_actual) { ['yes', 'yes', 'yes', 'no', 'no', 'no'] }
  
  let(:all_wrong_predictions) { ['A', 'A', 'A', 'A'] }
  let(:all_wrong_actual) { ['B', 'B', 'B', 'B'] }
  
  let(:single_class_predictions) { ['A', 'A', 'A', 'A'] }
  let(:single_class_actual) { ['A', 'A', 'A', 'A'] }
  ...
  describe "Accuracy Evaluation Tests" do
    ...
    context "accuracy edge cases" do
      ...
      it "test_mismatched_lengths" do
        ...
        expect {
          described_class.accuracy(['A', 'B'], ['A'])
        }.to raise_error(ArgumentError)
      end
    end
  end
    ...
    context "matrix generation" do
      it "test_confusion_matrix_binary" do
        ...
        if described_class.respond_to?(:confusion_matrix)
          ...
          matrix.each do |actual_class, row|
            expect(row).to be_a(Hash)
            expect(row.keys).to include('yes', 'no')
            
            row.each do |predicted_class, count|
      ...
      it "test_confusion_matrix_multiclass" do
        ...
        if described_class.respond_to?(:confusion_matrix)
          ...
          classes.each do |cls|
            ...
            matrix[cls].each do |predicted_cls, count|
              expect(count).to be_an(Integer)
              expect(count).to be >= 0
            end
          end
    ...
    context "k-fold validation" do
      it "test_cross_validation_setup" do
        ...
        if described_class.respond_to?(:cross_validate)
          ...
          dataset_items = [
            ['feature1', 'A'], ['feature2', 'B'], ['feature3', 'A'],
            ['feature4', 'B'], ['feature5', 'A'], ['feature6', 'B']
          ]
          ...
          allow(dummy_classifier).to receive(:build).and_return(dummy_classifier)
          allow(dummy_classifier).to receive(:eval).and_return('A')
    ...
    context "significance testing" do
      it "test_mcnemar_test" do
        ...
        if described_class.respond_to?(:mcnemar_test)
          classifier1_predictions = ['A', 'B', 'A', 'C', 'B']
          classifier2_predictions = ['A', 'B', 'B', 'C', 'A']
          actual_values = ['A', 'B', 'A', 'C', 'B']
  ...
  describe "Edge Case Tests" do
    context "unusual scenarios" do
      ...
      it "test_unseen_class_in_predictions" do
        ...
        actual_with_limited = ['A', 'A', 'B', 'B']
        predicted_with_extra = ['A', 'C', 'B', 'B']  # 'C' not in actual
