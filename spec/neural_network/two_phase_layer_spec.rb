...
RSpec.describe Ai4r::NeuralNetwork::TwoPhaseLayer do
  ...
  describe "TwoPhaseLayer Processing Tests" do
    ...
    context "backward pass" do
      ...
      it "test_weight_updates" do
        ...
        final_weights = layer.nodes.map { |node| node.weights }
  ...
  describe "Integration Tests" do
    it "provides complete layer functionality" do
      ...
      final_error = final_output.zip(test_output).map { |actual, expected| (expected - actual).abs }.sum
    ...
    it "maintains layer properties during training" do
      ...
      5.times do
        output = layer.eval(test_input)
        error = output.map { |v| rand(0.1) - 0.05 }  # Random small errors
        layer.backprop(error)
  ...
  def calculate_layer_error(output, target)
    output.zip(target).map { |actual, expected| (expected - actual) ** 2 }.sum / output.length
  end
end