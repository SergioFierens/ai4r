...
module Ai4r
  module NeuralNetwork
    ...
    class EnhancedNeuralNetwork
      ...
      def forward(inputs, training = true)
        ...
        @layers.each_with_index do |layer, layer_idx|
          ...
          # Add bias if enabled
          unless @configuration.disable_bias
            pre_activation = pre_activation.zip(layer[:bias]).map { |z, b| z + b }
          end
      ...
      def train(training_data, validation_data = nil, epochs = 100)
        ...
        epochs.times do |epoch|
          ...
          if validation_data
            ...
            if early_stopping && early_stopping.check(val_loss)
          ...
          # Interactive mode
          if @configuration.interactive && epoch % 10 == 0
            handle_interactive_mode(epoch)
          end
        end
      ...
      def evaluate(test_data)
        ...
        test_data.each do |sample|
          ...
          if @configuration.task_type == :classification
      ...
      def setup_regularization(reg_config)
        ...
        if reg_config[:dropout]
          @regularization.add_technique(Dropout.new(reg_config[:dropout][:rate] || 0.5))
        end
        
        if reg_config[:l1]
          @regularization.add_technique(L1Regularization.new(reg_config[:l1][:lambda] || 0.01))
        end
        
        if reg_config[:l2]
          @regularization.add_technique(L2Regularization.new(reg_config[:l2][:lambda] || 0.01))
        end
      end
      ...
      def print_weight_statistics
        ...
        @layers.each_with_index do |layer, idx|
          ...
          if weights.any? { |w| w.abs > 10 }
            puts "  ⚠️  Large weights detected"
          end
          
          if calculate_std(weights) < 0.01
            puts "  ⚠️  Low variance - check initialization"
          end
        end
      end
      ...
      def calculate_weight_gradients(layer_idx, output_gradients)
        # Simplified weight gradient calculation
        layer = @layers[layer_idx]
        Array.new(layer[:output_size]) { Array.new(layer[:input_size], 0.1) }
      end
      
      def calculate_input_gradients(layer_idx, output_gradients)
      ...
      def calculate_regularization_gradients(weights)
        ...
        weights.map { |row| row.map { |w| 0.0 } }  # Simplified
      end
      ...
      def update_layer_weights(layer_idx, weight_gradients, bias_gradients)
        ...
        # Update biases
        if bias_gradients && !@configuration.disable_bias
    ...
    class NetworkConfiguration
      attr_accessor :activation_function, :output_activation, :optimizer, :optimizer_params
      attr_accessor :loss_function, :task_type, :weight_initialization
      attr_accessor :regularization_config, :early_stopping, :early_stopping_patience
      attr_accessor :batch_size, :shuffle_data, :verbose, :interactive
      attr_accessor :print_every, :lr_scheduler, :disable_bias
    ...
    class EnhancedNetworkMonitor
      attr_reader :training_losses, :validation_losses, :validation_accuracies
      attr_reader :layer_activations, :layer_gradients, :gradient_norms
      attr_reader :training_time, :epoch_times
      ...
      def record_layer_activation(layer_idx, post_activation, pre_activation)
        while @layer_activations.length <= layer_idx
          @layer_activations << []
        end
      ...
      def record_layer_gradient(layer_idx, gradients)
        while @layer_gradients.length <= layer_idx
          @layer_gradients << []
        end
      ...
      def analyze_gradient_flow
        ...
        plot_curve(@gradient_norms[-20..-1] || @gradient_norms, "Gradient Norm")
      end
      ...
      def print_activation_statistics
        ...
        @layer_activations.each_with_index do |layer_acts, layer_idx|
          ...
          # Warnings
          if recent_acts[:dead_neurons] > 0
            puts "  ⚠️  Dead neurons detected"
          end
          
          if recent_acts[:saturation] > layer_acts.length * 0.1
            puts "  ⚠️  High saturation detected"
          end
        end
      end
