  ...
  module NeuralNetwork
    ...
    class Backpropagation
      ...
      def initialize(network_structure)
        ...
        @initial_weight_function = ->(_n, _i, _j) { ((rand 2000) / 1000.0) - 1 }
        @propagation_function = ->(x) { 1 / (1 + Math.exp(-1 * x)) } # lambda { |x| Math.tanh(x) }
        @derivative_propagation_function = ->(y) { y * (1 - y) } # lambda { |y| 1.0 - y**2 }
        @disable_bias = false
        @learning_rate = 0.25
        @momentum = 0.1
      end
      ...
      def eval(input_values)
        ...
        raise 'Activation nodes array is empty' if @activation_nodes.empty?
      ...
      def feedforward(input_values)
        raise 'Activation nodes array is empty' if @activation_nodes.empty?
        raise 'Input layer is empty' if @activation_nodes.first.nil?

        input_values.each_index do |input_index|
          if input_index >= @activation_nodes.first.length
            raise ArgumentError, "Input index #{input_index} out of bounds"
          end
        ...
        @weights.each_index do |n|
          ...
          @structure[n + 1].times do |j|
            ...
            @activation_nodes[n].each_index do |i|
              if i >= @weights[n].length || j >= @weights[n][i].length
                raise ArgumentError, 'Weight access out of bounds'
              end
      ...
      def init_activation_nodes
        ...
        @activation_nodes[0...-1].each { |layer| layer << 1.0 } unless disable_bias
      end
        ...
        (@activation_nodes.length - 2).downto(1) do |layer_index|
          ...
          @activation_nodes[layer_index].each_index do |j|
            ...
            layer_deltas[j] = (@derivative_propagation_function.call(
              @activation_nodes[layer_index][j]
            ) * error)
          end
