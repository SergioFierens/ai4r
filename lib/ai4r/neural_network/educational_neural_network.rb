...
module Ai4r
  module NeuralNetwork
    ...
    class EducationalNeuralNetwork
      ...
      def collect_network_info
        ...
        if @network_type == :hopfield
          @network_info[:patterns_stored] = @training_data&.length || 0
        end
      end
      ...
      def visualize_step(step_info)
        NeuralNetworkVisualizer.new(@network, @network_info, @training_history, @configuration).visualize_step(step_info)
      end
    end
    
    # Configuration class for neural network parameters
    class NeuralNetworkConfiguration
      attr_accessor :learning_rate, :momentum, :activation_function_name, :disable_bias
      attr_accessor :verbose, :explain_predictions, :convergence_threshold
      attr_accessor :max_iterations, :weight_initialization
    ...
    class NeuralNetworkMonitor
      attr_reader :start_time, :training_time, :error_history, :epoch_data
    ...
    class EducationalBackpropagation < Backpropagation
      ...
      def train_with_steps(training_data, epochs)
        ...
        epochs.times do |epoch|
          ...
          training_data.each_with_index do |(input, target), example_index|
            ...
            if example_index < 3 && epoch % 100 == 0
              weight_changes << capture_weight_snapshot
            end
          end
          ...
          # Record epoch data
          epoch_data = @monitor.record_epoch(epoch, avg_error, {
      ...
      def set_activation_function(function_name)
        case function_name
        when :sigmoid
          @propagation_function = lambda { |x| 1.0 / (1.0 + Math.exp(-x)) }
          @derivative_propagation_function = lambda { |y| y * (1.0 - y) }
        when :tanh
        ...
        when :relu
          ...
          @derivative_propagation_function = lambda { |y| y > 0 ? 1.0 : 0.0 }
        when :linear
          @propagation_function = lambda { |x| x }
          @derivative_propagation_function = lambda { |y| 1.0 }
        else
          # Default to sigmoid
          @propagation_function = lambda { |x| 1.0 / (1.0 + Math.exp(-x)) }
          @derivative_propagation_function = lambda { |y| y * (1.0 - y) }
    ...
    class NeuralNetworkVisualizer
      ...
      def visualize_step(step_info)
        ...
        if step_info[:error]
          puts "Training Error: #{step_info[:error].round(6)}"
        end
        
        if step_info[:weight_changes] && step_info[:weight_changes].any?
          puts "Weight changes recorded for analysis"
        end
      end
      ...
      def visualize_hopfield_network
        ...
        if @network_info[:patterns_stored] && @network_info[:structure]
      ...
      def visualize_network_structure(structure)
        ...
        structure.each_with_index do |layer_size, layer_index|
          ...
          # Print neurons
          layer_size.times do |neuron_index|
            print "  ◯"
          ...
          # Print connections
          if layer_index < structure.length - 1
      ...
      def visualize_training_progress
        ...
        if @training_history.length > 10
          ...
          sample_epochs.each do |index|
            ...
            if epoch_data && epoch_data[:error]
              puts sprintf("%5d | %8.6f", epoch_data[:epoch], epoch_data[:error])
            end
          end
        else
    ...
    class NeuralNetworkAnalyzer
      ...
      def analyze_weights
        ...
        if all_weights.any? { |w| w.abs > 10 }
          puts "  ⚠️  Large weights detected - potential instability"
        end
        
        if weight_std < 0.1
          puts "  ⚠️  Low weight variance - potential poor initialization"
        end
      end
      ...
      def analyze_complexity
        ...
        total_params = @structure.zip(@structure[1..-1] + [0]).sum do |input_size, output_size|
        ...
        if total_params > 1000
          puts "  ⚠️  High parameter count - ensure sufficient training data"
        end
      ...
      def suggest_improvements
        ...
        if @structure.length == 2
          puts "  • Add hidden layers for more complex pattern learning"
        end
