    ...
    class InteractiveClusteringExplorer
      ...
      def compare_algorithms_menu
        ...
        begin
          ...
          selected_algorithms = indices.filter_map { |i| algorithms[i - 1] }
      ...
      def visualize_results_menu
        ...
        # Show recent results
        recent_results = @session_history.reject { |h| h[:type] == :comparison }.last(5)
      ...
      def get_algorithm_parameters(algorithm_name)
        ...
        parameters.each do |param_name, param_config|
          ...
          if input.empty?
            ...
            begin
              params[param_name] = case param_config[:type]
                                   when :integer
                                     input.to_i
                                   when :float
                                     input.to_f
      ...
      def display_clustering_result(result)
        ...
        puts "Cluster sizes: #{stats[:cluster_sizes].join(', ')}" if stats[:cluster_sizes]
      ...
      def explore_single_parameter(param_name, param_config)
        ...
        results.each do |result|
          puts format('%-11s | %7.4f | %8d | %6.3fs',
                      result[:value].to_s, result[:quality], result[:clusters], result[:time])
        end

        # Find best value
        best = results.max_by { |r| r[:quality] }
        puts "\nBest parameter value: #{best[:value]} (quality: #{best[:quality].round(4)})" if best
      end
      ...
      def generate_gaussian_blobs
        ...
        dataset_name = :"custom_blobs_#{Time.now.to_i}"
        @framework.add_dataset(dataset_name, dataset,
                               "Custom Gaussian blobs: #{n_clusters} clusters, #{n_points} points")
      ...
      def generate_moons_dataset
        ...
        dataset_name = :"custom_moons_#{Time.now.to_i}"
      ...
      def generate_circles_dataset
        ...
        dataset_name = :"custom_circles_#{Time.now.to_i}"
      ...
      def generate_anisotropic_dataset
        ...
        dataset_name = :"custom_anisotropic_#{Time.now.to_i}"
      ...
      def generate_varied_density_dataset
        ...
        dataset_name = :"custom_density_#{Time.now.to_i}"
      ...
      def generate_noisy_dataset
        ...
        dataset_name = :"custom_noisy_#{Time.now.to_i}"
        @framework.add_dataset(dataset_name, dataset,
                               "Custom noisy: #{n_points} points, #{(noise_ratio * 100).round}% noise")
