  ...
  module Clusterers
    ...
    class ClusteringCurriculum
      ...
      def lesson_1_clustering_concepts
        ...
        @framework.run_algorithm(:k_means, :lesson1_simple,
                                 { k: 3, verbose: true })
      ...
      def lesson_2_distance_metrics
        ...
        @framework.run_algorithm(:k_means, :lesson2_elongated,
                                 { k: 2, verbose: false })
      ...
      def lesson_9_dbscan_introduction
        ...
        @framework.run_algorithm(:dbscan, :lesson6_moons,
        ...
        puts "→ DBSCAN found #{result2[:stats][:num_noise_points]} noise points" if result2[:stats][:num_noise_points]
      ...
      def lesson_10_algorithm_comparison
        ...
        test_datasets = %i[lesson1_simple lesson6_moons lesson6_circles lesson9_noisy]
        algorithms = %i[k_means hierarchical_average dbscan]
      ...
      def lesson_11_real_world_applications
        ...
        final_result[:algorithm_instance].clusters.each_with_index do |cluster, i|
          ...
          avg_age = cluster.data_items.sum { |c| c[0] } / cluster.data_items.length
          avg_income = cluster.data_items.sum { |c| c[1] } / cluster.data_items.length
          avg_spending = cluster.data_items.sum { |c| c[2] } / cluster.data_items.length
      ...
      def generate_customer_data(n_customers)
        ...
        data_items = []
        labels = %w[age income spending]

        # Create 4 customer segments
        segments = [
          { age: [25, 35], income: [30_000, 50_000], spending: [500, 1500] },    # Young, low income
          { age: [35, 50], income: [50_000, 80_000], spending: [1500, 3000] },   # Middle-aged, moderate
          { age: [50, 65], income: [70_000, 120_000], spending: [2000, 5000] },  # Older, high income
          { age: [20, 30], income: [60_000, 100_000], spending: [3000, 8000] }   # Young professionals
        ]
