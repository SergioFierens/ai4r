  ...
  module Data
    ...
    class DataVisualizer
      ...
      def histogram(column)
        ...
        counts.each_with_index do |count, i|
          ...
          puts format('%6.2f-%6.2f |%s %d', bin_start, bin_end, bar.ljust(40), count)
        end
      end
      ...
      def box_plot(column)
        ...
        puts line
        puts format('%-8.2f %40s %8.2f', min_val, '', max_val)
      end
      ...
      def scatter_plot(columns)
        ...
        grid.each_with_index do |row, i|
          ...
          puts format('%8.2f |%s', y_value, row.join)
        end

        puts "         #{'-' * width}"
        puts format('         %-8.2f %30s %8.2f', min1, col1, max1)
      ...
      def bar_chart(column)
        ...
        value_counts.sort_by { |_, count| -count }.each do |value, count|
          ...
          puts format("%-#{max_label_length}s |%s %d (%.1f%%)",
      ...
      def get_column_values(column)
        column_index = @dataset.get_index(column)
        @dataset.data_items.filter_map { |row| row[column_index] }
      end

      def get_numeric_column_names
        @dataset.data_labels.select.with_index do |_label, index|
    ...
    class CorrelationAnalyzer
      ...
      def calculate_correlations
        ...
        numeric_columns.combination(2).each do |col1, col2|
          ...
          next unless values1.length == values2.length && values1.length > 1
        ...
        if @config[:verbose]
          ...
          correlations.each_value do |data|
      ...
      def get_numeric_columns
        ...
        @dataset.data_labels.each_with_index do |label, index|
          values = @dataset.data_items.map { |row| row[index] }
          columns << { name: label, index: index } if values.any?(Numeric)
        end
        columns
      end
    ...
    class DistanceComparator
      ...
      def compare(sample_size)
        ...
        puts 'Pair            Euclidean    Manhattan    Chebyshev    Cosine      '
        puts '-' * 70
      ...
      def get_numeric_data
        ...
        @dataset.data_items.filter_map do |row|
    ...
    class FeatureEngineer
      ...
      def add_interaction_features(data_items)
        ...
        engineered_data = data_items.map do |row|
          ...
          numeric_features.combination(2) do |(f1, _i1), (f2, _i2)|
            interaction_features << (f1 * f2)
          end
      ...
      def add_binned_features(data_items)
        ...
        engineered_data = data_items.map do |row|
          ...
          binned_features = features.each_with_index.filter_map do |feature, col|
