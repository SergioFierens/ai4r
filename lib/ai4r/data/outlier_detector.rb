  ...
  module Data
    ...
    class OutlierDetector
      ...
      def comprehensive_analysis
        methods = [:iqr, :z_score, :modified_z_score]
      ...
      def isolation_forest_method(contamination = 0.1)
        ...
        if @verbose
          puts "🌲 Applying simplified isolation forest (contamination=#{contamination})"
        end
        
        @dataset.data_labels.each_with_index do |label, col_idx|
          values = @dataset.data_items.map { |row| row[col_idx] }.compact
      ...
      def find_consensus_outliers(methods_comparison)
        ...
        all_outlier_indices.each do |column, indices|
          consensus_indices = indices.select { |idx, methods| methods.length > 1 }
          
          if consensus_indices.any?
            consensus[column] = {
      ...
      def generate_outlier_recommendations(analysis)
        ...
        if total_outliers == 0
          recommendations << "No outliers detected. Data appears to be well-behaved."
        elsif total_outliers < @dataset.data_items.length * 0.05
          recommendations << "Low outlier rate (<5%). Investigate individual cases for data quality issues."
        elsif total_outliers < @dataset.data_items.length * 0.1
          recommendations << "Moderate outlier rate (5-10%). Consider robust preprocessing methods."
        else
          recommendations << "High outlier rate (>10%). Investigate data collection process and consider domain expertise."
        end
        ...
        if method_results.length > 1
          ...
          max_method = outlier_counts.max_by { |method, count| count }&.first
          min_method = outlier_counts.min_by { |method, count| count }&.first
