  ...
  module Data
    ...
    class EducationalDataExamples
      ...
      def self.data_exploration_tutorial(verbose: true)
        ...
        customer_data = [
          ...
          [25, 35000, 12, 8.5, "North", true],
          [nil, 42000, 8, 7.2, "South", true],      # Missing age
          [35, nil, 15, 9.1, "East", true],         # Missing income
          [28, 38000, 10, 8.8, "North", true],
          [45, 65000, 25, 6.5, "West", false],
          [33, 48000, 18, 8.9, "East", true],
          [29, 41000, 14, 7.8, "South", true],
          [52, 75000, 30, 5.2, "West", false],      # Potential outlier
          [26, 36000, 11, "", "North", true],       # Missing satisfaction
          [31, 44000, 16, 8.3, "East", true],
          [38, 58000, 22, 7.9, "West", true],
          [27, 39000, 13, 8.6, "South", true],
          [200, 45000, 17, 8.1, "North", true],     # Age outlier
          [34, 51000, 19, 9.0, "East", true],
          [30, 43000, 15, 8.4, "South", true]
        ]
        
        labels = ["age", "income", "purchases", "satisfaction", "region", "is_customer"]
      ...
      def self.preprocessing_pipeline_tutorial(verbose: true)
        ...
        labels = ["feature1", "feature2", "feature3", "category", "target"]
        ...
        outliers = dataset.detect_outliers(method: :z_score, threshold: 2.5)
      ...
      def self.feature_engineering_tutorial(verbose: true)
        ...
        dataset.visualize(["x1", "x2"], :scatter)
      ...
      def self.correlation_analysis_tutorial(verbose: true)
        ...
        size.times do |i|
          ...
          x5 = x1**2 / 10 + rand * 1 - 0.5 # Non-linear relationship
          
          correlation_data << [x1, x2, x3, x4, x5, (x1 + x2 > 10 ? 1 : 0)]
        end
        
        labels = ["x1_base", "x2_positive", "x3_negative", "x4_random", "x5_nonlinear", "target"]
        ...
        dataset.visualize(["x1_base", "x2_positive"], :scatter)
        dataset.visualize(["x1_base", "x3_negative"], :scatter)
        dataset.visualize(["x1_base", "x4_random"], :scatter)
      ...
      def self.data_quality_comprehensive(verbose: true)
        ...
        problematic_data = [
          [25, 50000, "Engineer", "New York", 1],
          [25, 50000, "Engineer", "New York", 1],    # Duplicate
          [30, 60000, "Doctor", "Boston", 1],
          [nil, 55000, "Teacher", "Chicago", 0],     # Missing age
          [35, nil, "Lawyer", "Seattle", 1],         # Missing salary
          [28, 48000, "", "Miami", 0],               # Missing profession
          [150, 45000, "Artist", "Portland", 0],     # Age outlier
          [32, -5000, "Writer", "Austin", 1],        # Impossible salary
          [29, 52000, "Nurse", "Denver", 1],
          [31, 58000, "Chef", "Nashville", 0],
          [27, 51000, "Designer", "", 1],            # Missing city
          [33, 62000, "Manager", "Phoenix", 1],
          [26, 49000, "Analyst", "Atlanta", 0],
          [34, 0, "Student", "Philadelphia", 0],     # Zero salary (student)
          [28, 53000, "Developer", "San Diego", 1],
          [nil, nil, "", "", nil]                    # Completely empty row
        ]
        
        labels = ["age", "salary", "profession", "city", "employed"]
        ...
        iqr_outliers = dataset.detect_outliers(method: :iqr, threshold: 1.5)
        z_outliers = dataset.detect_outliers(method: :z_score, threshold: 2.0)
      ...
      def self.generate_classification_data(size: 100, features: 2, classes: 2)
        ...
        size.times do
          # Generate features
          feature_values = features.times.map { rand * 10 }
    ...
    class DataGenerator
      ...
      def self.generate_regression_data(size, features, noise)
        ...
        size.times do
          feature_values = features.times.map { rand * 10 - 5 }  # Range [-5, 5]
      ...
      def self.generate_clustering_data(size, features, noise)
        ...
        size.times do
          ...
          feature_values = features.times.map do |i|
