  ...
  module Data
    ...
    class EducationalDataExamples
      ...
      def self.data_exploration_tutorial(verbose: false)
        ...
        if dataset.respond_to?(:missing_values_report)
          results[:missing_analysis] = dataset.missing_values_report
        else
          results[:missing_analysis] = analyze_missing_values_basic(dataset)
        end
      ...
      def self.data_quality_comprehensive(verbose: false)
        ...
        if verbose
          puts "\n🔍 Analyzing Missing Values Patterns..."
        end
      ...
      def self.preprocessing_pipeline_tutorial(verbose: false)
        ...
        recommendations = feature_scaler.recommend_scaling_method(:random_forest)
        ...
        engineered_data = feature_engineer.apply_techniques(dataset.data_items, [:polynomial, :interaction])
        dataset = create_dataset_from_items(engineered_data, dataset.data_labels)
        results[:preprocessing_steps] << { step: "feature_engineering", techniques: ["polynomial", "interaction"] }
      ...
      def self.feature_engineering_tutorial(verbose: false)
        ...
        techniques = [:polynomial, :interaction, :log_transform, :binning]
        
        techniques.each do |technique|
          if verbose
            puts "\n⚗️  Testing #{technique.to_s.humanize} Features"
          end
          ...
          if verbose
            puts "  Added #{results[:techniques][technique][:added_features]} new features"
          end
        end
        
        # Combined approach
        if verbose
          puts "\n🚀 Combined Feature Engineering Approach"
        end
        
        combined_data = feature_engineer.apply_techniques(dataset.data_items, [:polynomial, :interaction])
      ...
      def self.cross_validation_tutorial(verbose: false)
        ...
        if verbose
          puts "\n⚖️  Method 3: Stratified Splitting"
        end
        ...
        if verbose
          puts "\n🎯 Method 4: Holdout Validation (Train/Val/Test)"
        end
      ...
      def self.distance_metrics_tutorial(verbose: false)
        ...
        distance_methods = [:euclidean, :manhattan, :chebyshev, :cosine]
        
        distance_methods.each do |method|
          if verbose
            puts "\n📏 Testing #{method.to_s.humanize} Distance"
          end
      ...
      def self.create_customer_dataset_with_issues
        ...
        200.times do |i|
          ...
          income = case rand
                   when 0..0.8 then rand(25000..85000)
                   when 0.8..0.95 then rand(85000..150000)
                   else rand(200000..500000)  # Outliers
                   end
        ...
        labels = ["age", "income", "purchase_amount", "category", "satisfaction"]
      ...
      def self.create_problematic_dataset
        ...
        labels = ["patient_id", "age", "weight", "bp_systolic", "bp_diastolic", "diagnosis"]
        EnhancedDataSet.new(data_items: data_items, data_labels: labels)
      end
      ...
      def self.create_messy_real_world_dataset
        ...
        150.times do |i|
          ...
          price = case rand
                  when 0..0.85 then rand(100000..800000)
                  when 0.85..0.95 then rand(800000..1500000)
                  else rand(2000000..5000000)  # Luxury outliers
                  end
          
          # Location with inconsistent formatting
          location = ["Downtown", "Suburb", "Rural", "DOWNTOWN", "suburb"].sample
        ...
        labels = ["square_feet", "bedrooms", "bathrooms", "price", "location", "year_built"]
        EnhancedDataSet.new(data_items: data_items, data_labels: labels)
      end
      ...
      def self.create_classification_dataset
        ...
        labels = ["feature1", "feature2", "feature3", "class"]
        EnhancedDataSet.new(data_items: data_items, data_labels: labels)
      end
      ...
      def self.calculate_basic_statistics(dataset)
        ...
        dataset.data_labels.each_with_index do |label, idx|
          values = dataset.data_items.map { |row| row[idx] }.compact
          ...
          if numeric_values.any?
            ...
            stats[label] = {
              ...
              most_common: values.group_by(&:itself).max_by { |k, v| v.length }&.first
            }
          end
        end
      ...
      def self.analyze_data_types(dataset)
        ...
        dataset.data_labels.each_with_index do |label, idx|
          values = dataset.data_items.map { |row| row[idx] }.compact.first(20)
          ...
          if values.all? { |v| v.is_a?(Numeric) }
            ...
            unique_count = values.uniq.length
            if unique_count <= [values.length * 0.1, 10].max
              types[label] = :categorical
            else
              types[label] = :text
            end
          end
      ...
      def self.display_exploration_insights(results)
        ...
        numeric_columns = results[:data_types].count { |_, type| [:integer, :float].include?(type) }
        ...
        if results[:statistics].any?
          ...
          if ranges.any?
            ...
            if large_range_cols.any?
              puts "  • Large value ranges detected - consider scaling"
            end
          end
        end
      end
      ...
      def self.check_data_consistency(dataset)
        ...
        dataset.data_labels.each_with_index do |label, idx|
          values = dataset.data_items.map { |row| row[idx] }.compact.uniq
          if values.length == 1
            issues << "Column '#{label}' has constant value"
          end
        end
      ...
      def self.cap_outliers(data_items, outliers_info)
        ...
        outliers_info.each do |column, info|
      ...
      def self.generate_correlation_insights(correlations, strong_correlations)
        ...
        if correlations.any?
          avg_abs_corr = correlations.values.flat_map(&:values).map(&:abs).sum / correlations.values.flat_map(&:values).length
