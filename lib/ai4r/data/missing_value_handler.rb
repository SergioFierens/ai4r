  ...
  module Data
    ...
    class MissingValueHandler
      ...
      def analyze_missing_patterns(data_items, data_labels)
        ...
        if @verbose
          display_missing_analysis(analysis)
        end
      ...
      def explain_missing_value_strategy(strategy)
        ...
        if @config[:show_warnings]
          show_strategy_warnings(strategy)
        end
      end
      ...
      def linear_interpolation(data_items)
        ...
        num_columns.times do |col_idx|
          ...
          if values.any? { |v| !missing_value?(v) && v.is_a?(Numeric) }
      ...
      def knn_imputation(data_items, k = 5)
        ...
        data_items.each_with_index do |row, row_idx|
          ...
          row.each_with_index do |cell, col_idx|
            if missing_value?(cell)
              neighbor_values = neighbors.map { |neighbor| neighbor[col_idx] }.compact
              
              if neighbor_values.any? && neighbor_values.all? { |v| v.is_a?(Numeric) }
                ...
                filled_data[row_idx][col_idx] = neighbor_values.group_by(&:itself).max_by { |k, v| v.length }&.first
              end
            end
          end
        end
      ...
      def calculate_column_means(data_items)
        ...
        num_columns.times do |col_idx|
          values = data_items.map { |row| row[col_idx] }.compact
          numeric_values = values.select { |v| v.is_a?(Numeric) }
          
          if numeric_values.any?
            means << (numeric_values.sum.to_f / numeric_values.length)
          else
            means << nil
          end
        end
      ...
      def calculate_column_modes(data_items)
        ...
        num_columns.times do |col_idx|
          ...
          if values.any?
            mode = values.group_by(&:itself).max_by { |k, v| v.length }&.first
            modes << mode
          else
            modes << nil
          end
      ...
      def fill_missing_with_values(data_items, fill_values, method_name)
        filled_data = data_items.map do |row|
      ...
      def count_filled_values(original_data, filled_data)
        ...
        original_data.each_with_index do |row, row_idx|
          row.each_with_index do |cell, col_idx|
            if missing_value?(cell) && !missing_value?(filled_data[row_idx][col_idx])
              count += 1
            end
          end
        end
      ...
      def detect_missing_patterns(data_items, data_labels)
        ...
        data_items.each_with_index do |row, row_idx|
          missing_count = row.count { |cell| missing_value?(cell) }
          if missing_count > row.length * 0.5
            high_missing_rows += 1
          end
        end
        
        if high_missing_rows > 0
          patterns << "#{high_missing_rows} rows have >50% missing values"
        end
      ...
      def generate_missing_value_recommendations(analysis, data_items)
        ...
        if missing_percentage < 5
          recommendations << "Low missing data (<5%). Simple strategies like mean/mode imputation should work well."
        elsif missing_percentage < 20
          recommendations << "Moderate missing data (5-20%). Consider median/mode imputation or more sophisticated methods."
        else
          recommendations << "High missing data (>20%). Investigate data collection process. Consider domain-specific imputation."
        end
      ...
      def display_missing_analysis(analysis)
        ...
        if analysis[:by_row].any?
          puts "\nRows with missing values: #{analysis[:by_row].length}"
        end
