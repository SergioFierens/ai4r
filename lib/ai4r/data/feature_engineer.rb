  ...
  module Data
    ...
    class FeatureEngineer
      ...
      def suggest_techniques(feature_analysis, target_column = nil)
        ...
        suggestions.concat(suggest_target_based_techniques(feature_analysis, target_column)) if target_column

        # Remove duplicates and prioritize
        suggestions.uniq! { |s| s[:name] }
        suggestions.sort_by! { |s| -s[:priority] }

        display_technique_suggestions(suggestions) if @verbose
      ...
      def add_polynomial_features(data_items, degree = 2)
        ...
        puts "🔢 Adding polynomial features (degree #{degree})" if @verbose
        ...
        data_items.each do |item|
          ...
          if degree >= 2
            numeric_indices.each do |idx|
              value = item[idx]
              new_item << (value**2) if value.is_a?(Numeric)
            end
          end
          ...
          if degree >= 3
            numeric_indices.each do |idx|
              value = item[idx]
              new_item << (value**3) if value.is_a?(Numeric)
            end
          end
      ...
      def add_interaction_features(data_items)
        ...
        puts '🤝 Adding interaction features' if @verbose
        ...
        data_items.each do |item|
          ...
          numeric_indices.combination(2).each do |idx1, idx2|
            ...
            new_item << (val1 * val2) if val1.is_a?(Numeric) && val2.is_a?(Numeric)
          end
      ...
      def add_binned_features(data_items, bins = 5)
        ...
        puts "🗂️  Adding binned features (#{bins} bins)" if @verbose
      ...
      def add_log_features(data_items)
        ...
        puts '📈 Adding logarithmic features' if @verbose
        ...
        data_items.each do |item|
          ...
          item.each_with_index do |value, _idx|
      ...
      def add_sqrt_features(data_items)
        ...
        puts '√ Adding square root features' if @verbose
        ...
        data_items.each do |item|
          ...
          item.each_with_index do |value, _idx|
            new_item << Math.sqrt(value) if value.is_a?(Numeric) && value >= 0
          end
      ...
      def add_reciprocal_features(data_items)
        ...
        puts '🔄 Adding reciprocal features (1/x)' if @verbose
        ...
        data_items.each do |item|
          ...
          item.each_with_index do |value, _idx|
            new_item << (1.0 / value) if value.is_a?(Numeric) && value != 0
          end
      ...
      def add_exponential_features(data_items)
        ...
        puts '📊 Adding exponential features' if @verbose
        ...
        data_items.each do |item|
          ...
          item.each_with_index do |value, _idx|
            new_item << Math.exp(value) if value.is_a?(Numeric) && value.abs < 10 # Prevent overflow
          end
      ...
      def add_trigonometric_features(data_items)
        ...
        puts '〰️ Adding trigonometric features (sin, cos)' if @verbose
      ...
      def calculate_bin_boundaries(data_items, bins)
        ...
        num_features.times do |feature_idx|
          values = data_items.filter_map { |row| row[feature_idx] }
      ...
      def assign_to_bin(value, boundaries)
        ...
        (0...(boundaries.length - 1)).each do |i|
          return i if value.between?(boundaries[i], boundaries[i + 1])
        end
      ...
      def suggest_datetime_techniques(feature, _analysis)
        suggestions = []
      ...
      def suggest_text_techniques(feature, _analysis)
        suggestions = []
      ...
      def suggest_target_based_techniques(feature_analysis, _target_column)
        suggestions = []

        # Suggest interaction features based on target correlation
        high_correlation_features = feature_analysis.select do |_feature, analysis|
        ...
        numeric_features = feature_analysis.select { |_f, a| a[:type] == :numeric }
      ...
      def variance_based_selection(data_items)
        ...
        data_items.first.length.times do |feature_idx|
          ...
          next unless numeric_values.length > 1
      ...
      def calculate_mutual_information(data_items, feature_idx, target_idx)
        ...
        feature_values = data_items.filter_map { |row| row[feature_idx] }
        target_values = data_items.filter_map { |row| row[target_idx] }
        ...
        joint_freq.each do |(f, t), count|
          ...
          mi += p_ft * Math.log2(p_ft / (p_f * p_t)) if p_ft > 0 && p_f > 0 && p_t > 0
        end
