  ...
  module Data
    ...
    class FeatureScaler
      ...
      def recommend_scaling_method(target_algorithm = nil)
        ...
        if @verbose
          display_scaling_recommendations(recommendations, analysis)
        end
      ...
      def quantile_normal_transform(data_items)
        ...
        normal_data = uniform_data.map do |item|
          item.map do |value|
      ...
      def extract_numeric_values(data_items, feature_idx)
        data_items.map { |row| row[feature_idx] }.compact.select { |v| v.is_a?(Numeric) }
      end
      ...
      def calculate_skewness(values)
        ...
        skewness = values.sum { |v| ((v - mean) / std) ** 3 } / values.length
        skewness
      end
      ...
      def analyze_feature_characteristics
        ...
        num_features.times do |feature_idx|
          ...
          skewness = calculate_skewness(values)
          if skewness.abs > 1.0
            analysis[:skewed_features] << { index: feature_idx, skewness: skewness }
          end
