  ...
  module Data
    ...
    class EducationalDataSet < DataSet
      ...
      def initialize(options = {})
        ...
        analyze_data if !@data_items.empty?
      end
      ...
      def update_labels_after_engineering(techniques)
        ...
        if techniques.include?(:polynomial)
    ...
    class DataMetadata
      ...
      def detect_type(data_items, index)
        sample_values = data_items.first(100).map { |item| item[index] }.compact
        return :unknown if sample_values.empty?
        
        # Check if all values are numeric
        if sample_values.all? { |val| val.is_a?(Numeric) }
      ...
      def calculate_basic_stats(data_items, index)
        values = data_items.map { |item| item[index] }.compact
        ...
        if values.first.is_a?(Numeric) && values.length > 0
          stats.merge!({
            min: values.min,
            max: values.max,
    ...
    class DataQualityAnalyzer
      ...
      def detect_quality_issues
        ...
        @dataset.data_labels.each_with_index do |label, index|
          values = @dataset.data_items.map { |item| item[index] }.compact.uniq
          if values.length == 1
            issues << "Column '#{label}' has constant value (#{values.first})"
          end
        end
        ...
        if @dataset.data_items.length < 100
          @dataset.data_labels.each_with_index do |label, index|
            unique_count = @dataset.data_items.map { |item| item[index] }.compact.uniq.length
    ...
    class DataProfiler
      ...
      def statistical_summary
        ...
        puts sprintf("%-15s %8s %8s %8s %8s %8s", "Column", "Count", "Mean", "Std", "Min", "Max")
        puts "-" * 70
        
        @dataset.data_labels.each_with_index do |label, index|
          if @dataset.metadata.attribute_types[label] == :float || 
             @dataset.metadata.attribute_types[label] == :integer
            
            stats = @dataset.metadata.attribute_stats[label]
            values = @dataset.data_items.map { |item| item[index] }.compact
            ...
            puts sprintf("%-15s %8d %8.2f %8.2f %8.2f %8.2f", 
