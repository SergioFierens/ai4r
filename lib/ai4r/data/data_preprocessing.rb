  ...
  module Data
      ...
      def fill_with_statistic(data_items, stat_type)
        ...
        num_cols.times do |col|
          ...
          fill_values[col] = case stat_type
                             when :mean
                               values.select { |v| v.is_a?(Numeric) }.sum.to_f / values.length if values.any?(Numeric)
                             when :median
        ...
        puts "Filled #{filled_count} missing values using #{stat_type}" if @config[:verbose]
      ...
      def interpolate_missing(data_items)
        ...
        num_cols.times do |col|
          ...
          data_items.each_with_index do |row, idx|
            next unless row[col].nil? || row[col] == ''

            # Find surrounding valid values
            before = numeric_values.reverse.find { |i, _| i < idx }
            after = numeric_values.find { |i, _| i > idx }
    ...
    class OutlierDetector
      ...
      def iqr_method(threshold)
        ...
        @dataset.data_labels.each_with_index do |label, col|
          ...
          column_outliers = @dataset.data_items.each_with_index.select do |row, _idx|
            val = row[col]
            val.is_a?(Numeric) && (val < lower_bound || val > upper_bound)
          end.map(&:last)

          next unless column_outliers.any?
      ...
      def isolation_forest_method(threshold)
        ...
        @dataset.data_labels.each_with_index do |label, col|
          ...
          values.each_with_index do |val, _original_idx|
            ...
            column_outliers << @dataset.data_items.index { |row| row[col] == val } if isolation_score > threshold
          end
      ...
      def report_outliers(outliers, method)
        ...
        outliers.each do |column, info|
          ...
          puts "  Row indices: #{info[:indices].inspect}" if info[:count] <= 10 # Show indices for small numbers
        end
      end
    end
    ...
    class DataSplitter
      ...
      def split(test_size, random_seed = nil)
        ...
        test_data = data_items[split_index..]

        puts "Split: #{train_data.length} training, #{test_data.length} testing samples" if @config[:verbose]
      ...
      def k_fold_split(k, random_seed = nil)
        ...
        k.times do |i|
          ...
          train_data = data_items[0...start_idx] + data_items[end_idx..]
