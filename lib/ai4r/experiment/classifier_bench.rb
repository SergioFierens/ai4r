    ...
    class ClassifierBench
      ...
      def run(dataset, options = {})
        ...
        @classifiers.each do |name, classifier_info|
          ...
          if @educational_mode
            display_progress_insights(name, @results[name])
          end
        end
      ...
      def benchmark_classifier(name, classifier, dataset, options = {})
        results = {
          metrics: {},
          confusion_matrix: {},
          timing: {},
        ...
        if has_numeric_features?(dataset) && !supports_numeric_features?(classifier)
          ...
          results[:errors] << { type: :compatibility_warning, message: "Classifier may not handle numeric features well" }
        end
        ...
        results[:timing] = {
          avg_training_time: cv_results.map { |r| r[:training_time] }.sum / cv_results.size,
          avg_prediction_time: cv_results.map { |r| r[:prediction_time] }.sum / cv_results.size,
          total_time: cv_results.map { |r| r[:training_time] + r[:prediction_time] }.sum
        }
      ...
      def create_stratified_folds(dataset, k)
        ...
        class_groups.each { |_, indices| indices.shuffle! }
        
        # Distribute to folds
        folds = Array.new(k) { [] }
        class_groups.each do |class_label, indices|
      ...
      def calculate_metrics(actuals, predictions)
        ...
        classes.each do |cls|
          ...
          tn = actuals.zip(predictions).count { |a, p| a != cls && p != cls }
        ...
        # Macro averages
        macro_precision = class_metrics.values.map { |m| m[:precision] }.sum / classes.size
        macro_recall = class_metrics.values.map { |m| m[:recall] }.sum / classes.size
        macro_f1 = class_metrics.values.map { |m| m[:f1_score] }.sum / classes.size
        
        # Weighted averages
        total_support = class_metrics.values.map { |m| m[:support] }.sum
        weighted_precision = class_metrics.map { |c, m| m[:precision] * m[:support] }.sum / total_support
        weighted_recall = class_metrics.map { |c, m| m[:recall] * m[:support] }.sum / total_support
        weighted_f1 = class_metrics.map { |c, m| m[:f1_score] * m[:support] }.sum / total_support
      ...
      def detect_feature_types(dataset)
        ...
        # Return summary
        if feature_types.all? { |t| t == :numeric }
          :all_numeric
        elsif feature_types.all? { |t| t == :categorical }
          :all_categorical
        else
          :mixed
        end
      ...
      def display_accuracy_comparison(results)
        ...
        sorted_results.each_with_index do |(name, result), idx|
          ...
          trophy = idx == 0 ? "🥇" : (idx == 1 ? "🥈" : (idx == 2 ? "🥉" : "  "))
          
          puts sprintf("%s %-20s %s %.1f%% (±%.1f%%)",
            trophy,
            @classifiers[name][:friendly_name],
      ...
      def display_performance_comparison(results)
        puts "\n📈 Performance Metrics:"
        puts "-" * 80
        puts sprintf("%-20s %10s %10s %10s %10s",
          "Classifier", "Precision", "Recall", "F1-Score", "Accuracy")
        puts "-" * 80
        
        results.each do |name, result|
          puts sprintf("%-20s %10.3f %10.3f %10.3f %10.3f",
            @classifiers[name][:friendly_name],
      ...
      def display_timing_comparison(results)
        ...
        sorted_results.each do |name, result|
          ...
          puts sprintf("%-20s Training: %6.3fs  Prediction: %6.3fs",
            @classifiers[name][:friendly_name],
            training_time,
            prediction_time
          )
      ...
      def generate_classifier_insights(name, result)
        insights = []
        ...
        if accuracy > 0.9
          insights << "  ✨ Excellent performance (>90% accuracy)"
        elsif accuracy > 0.8
          insights << "  ✓ Good performance (>80% accuracy)"
        elsif accuracy > 0.7
          insights << "  → Moderate performance (>70% accuracy)"
        else
          insights << "  ⚠️  Poor performance (<70% accuracy)"
        end
        ...
        if cv_scores.any?
          ...
          if cv_std < 0.02
            insights << "  📊 Very stable across folds (low variance)"
          elsif cv_std < 0.05
            insights << "  📊 Reasonably stable across folds"
          else
            insights << "  📊 High variance across folds - may be overfitting"
          end
        end
        ...
        if total_time < 0.1
          insights << "  ⚡ Lightning fast (<0.1s)"
        elsif total_time < 1.0
          insights << "  🏃 Fast execution (<1s)"
        else
          insights << "  🐌 Slow execution (>1s)"
        end
      ...
      def display_progress_insights(name, result)
        accuracy = result[:metrics][:accuracy] || 0.0
...
<body>
  ...
  <pre>#{generate_insights}</pre>
      ...
      def has_numeric_features?(dataset)
        ...
        (0...(dataset.data_labels.size - 1)).each do |i|
          values = samples.map { |item| item[i] }
          return true if values.any? { |v| v.is_a?(Numeric) }
        end
