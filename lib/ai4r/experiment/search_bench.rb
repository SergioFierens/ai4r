  ...
  module Experiment
    ...
    class SearchBench
      ...
      def run(options = {})
        ...
        @algorithms.each do |algo_name, algo_info|
          ...
          @problems.each do |prob_name, prob_info|
            ...
            display_progress_insights(algo_name, prob_name, @results[algo_name][prob_name]) if @educational_mode
          end
        end
      ...
      def validate_pathfinding_problem(problem)
        required_keys = %i[grid start goal]
      ...
      def validate_optimization_problem(problem)
        required_keys = %i[chromosome_class population_size generations]
        required_keys.each do |key|
      ...
      def validate_setup
        raise 'No algorithms added' if @algorithms.empty?
        raise 'No problems added' if @problems.empty?
      end

      # Benchmark a specific algorithm/problem combination
      def benchmark_combination(_algo_name, algo_info, _prob_name, prob_info, _options)
        result = {
          solution: nil,
          success: false,
          search_time: 0.0,
      ...
      def execute_optimization(algorithm, _problem)
      ...
      def display_performance_comparison(results)
        ...
        puts '-' * 70
        puts 'Algorithm            Avg Time (s)    Avg Nodes       Success Rate   '
        puts '-' * 70
      ...
      def display_winner_analysis(results)
        ...
        fastest_times = {}
        results.each do |algo_name, algo_results|
          avg_time = algo_results.values.sum { |r| r[:search_time] } / algo_results.size
      ...
      def generate_algorithm_insights(_algo_name, algo_results)
        insights = []
        ...
        insights << if avg_time < 0.001
                      '  ⚡ Lightning fast execution (<1ms)'
                    elsif avg_time < 0.01
                      '  🏃 Fast execution (<10ms)'
                    elsif avg_time < 0.1
                      '  → Moderate execution time (<100ms)'
                    else
                      '  🐌 Slow execution (>100ms)'
                    end

        # Efficiency insights
        insights << if avg_nodes < 50
                      '  🧠 Highly efficient (explores few nodes)'
                    elsif avg_nodes < 200
                      '  📊 Moderately efficient'
                    else
                      '  📈 Explores many nodes (less efficient)'
                    end
      ...
      def generate_problem_insights(prob_name, results)
        ...
        problem_results = results.map { |_algo_name, algo_results| algo_results[prob_name] }
        successful_results = problem_results.select { |r| r[:success] }

        insights << if successful_results.empty?
                      '  ❌ No algorithm could solve this problem'
                    elsif successful_results.size == problem_results.size
                      '  ✅ All algorithms solved this problem'
                    else
                      '  ⚠️  Only some algorithms could solve this problem'
                    end
        ...
        if successful_results.any?
          ...
          insights << if time_variance > 0.1
                        '  📊 High performance variance between algorithms'
                      else
                        '  📊 Similar performance across algorithms'
                      end
        end
      ...
      def generate_comparative_insights(results)
        ...
        total_successes = results.sum { |_, algo_results| algo_results.values.count { |r| r[:success] } }
        total_attempts = results.size * @problems.size

        insights << if total_successes == total_attempts
                      '• All algorithm/problem combinations were successful'
                    elsif total_successes > total_attempts * 0.8
                      '• Most combinations were successful (>80%)'
                    else
                      '• Many combinations failed - check algorithm/problem compatibility'
                    end
      ...
      def generate_learning_recommendations(_results)
        recommendations = []
      ...
      def display_progress_insights(_algo_name, _prob_name, result)
        if result[:success]
        ...
        <<~HTML
          ...
          #{'  '}
            ...
            <table>
              ...
              #{@results.map do |algo_name, algo_results|
                algo_results.map do |prob_name, result|
                  "<tr>
