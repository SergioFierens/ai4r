    ...
    class Configuration
      ...
      def initialize(preset = :default, **options)
        ...
        options.each do |key, value|
          raise ArgumentError, "Unknown configuration parameter: #{key}" unless respond_to?("#{key}=")
      ...
      def validate!
        ...
        raise ArgumentError, 'Time limit must be positive' if @time_limit && @time_limit <= 0

        raise ArgumentError, 'Elitism rate must be less than 1.0' if @elitism_rate >= 1.0

        raise ArgumentError, 'Population size must be at least 2' if @population_size < 2
      end
      ...
      def with(**options)
        ...
        # Apply modifications
        options.each do |key, value|
          raise ArgumentError, "Unknown configuration parameter: #{key}" unless new_config.respond_to?("#{key}=")
      ...
      def explain_parameters
        ...
        explanations.each do |param, explanation|
          ...
          puts "#{param.to_s.capitalize.tr('_', ' ')}: #{explanation}"
      ...
      def validate_probability(param, message)
        value = instance_variable_get("@#{param}")
        raise ArgumentError, message unless value.between?(0, 1)
      end
    end
  end
end
