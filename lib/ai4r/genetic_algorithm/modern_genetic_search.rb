  ...
  module GeneticAlgorithm
    ...
    class ModernGeneticSearch
      ...
      def run(chromosome_class, *args)
        ...
        puts @config.to_s if @config.verbose
        ...
        while !termination_criteria_met?
          ...
          parents.each_slice(2) do |parent1, parent2|
            ...
            if rand < @config.crossover_rate
              ...
              offspring.concat([parent1.clone, parent2.clone])
            end
          end
          ...
          offspring.each do |individual|
            ...
            if mutated.fitness != original_fitness
              mutated_count += 1
            end
          end
      ...
      def validate_chromosome_class(chromosome_class)
        unless chromosome_class < Chromosome
          raise ArgumentError, "Chromosome class must inherit from Chromosome"
        end
      ...
      def termination_criteria_met?
        ...
        # Time limit reached
        if @config.time_limit && @start_time
          return true if Time.now - @start_time > @config.time_limit
        end
        
        # Fitness goal reached
        if @config.fitness_goal && !@population.empty?
          return true if @population.max_by(&:fitness).fitness >= @config.fitness_goal
        end
