...
module Ai4r
  module GeneticAlgorithm
    ...
    class VisualizationTools
      ...
      def self.plot_ascii_chart(title, x_data, y_data, width = 60, height = 15)
        puts "\n#{title}:"
      ...
      def self.analyze_diversity_trend(diversity_data)
        ...
        last_quarter_avg = diversity_data[-quarter_size..-1].sum / quarter_size
      ...
      def self.analyze_operator_performance(sorted_results)
        ...
        avg_generations = sorted_results.map { |_, data| data[:generations] }.sum / sorted_results.length
        fast_convergers = sorted_results.select { |_, data| data[:generations] < avg_generations }
        
        if fast_convergers.length > 0
          puts "• Fast converging operators: #{fast_convergers.map(&:first).join(', ')}"
        end
      end
      ...
      def self.provide_diversity_recommendations(initial, final, minimum)
        ...
        if minimum < 0.05
          puts "• Population nearly converged - consider re-diversification"
        end
      end
