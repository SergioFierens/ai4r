  ...
  module GeneticAlgorithm
    ...
    class EducationalGeneticSearch < ModernGeneticSearch
      ...
      def explain_fitness_function(chromosome_class, *args)
        ...
        puts "Example chromosome: #{example.genes[0..4].inspect}#{example.genes.length > 5 ? '...' : ''}"
      ...
      def adjust_mutation_rate
        ...
        @config.mutation_rate = input if input >= 0 && input <= 1
      end
      
      def adjust_crossover_rate
        print "Enter new crossover rate (current: #{@config.crossover_rate}): "
        input = gets.chomp.to_f
        @config.crossover_rate = input if input >= 0 && input <= 1
      end
      ...
      def constraint_handling_example(difficulty)
      ...
      def multi_objective_example(difficulty)
      ...
      def dynamic_optimization_example(difficulty)
    ...
    class ConceptTracker
      ...
      def learning_progress
        total_concepts = [:selection, :crossover, :mutation, :fitness, :population, 
