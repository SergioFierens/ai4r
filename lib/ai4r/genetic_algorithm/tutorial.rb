  ...
  module GeneticAlgorithm
    ...
    class GATutorial
      ...
      def run_custom_tutorial
        ...
        available_topics.each_with_index do |(key, name), index|
          puts "#{index + 1}. #{name}"
        end
        ...
        selected_topics = topic_indices.map { |i| available_topics.keys[i - 1] }.compact
      ...
      def lesson_simple_example_onemax
        ...
        result = ga.run_tutorial(Examples::OneMaxChromosome, 8)
      ...
      def lesson_putting_it_together
        ...
        result = ga.run_step_by_step(Examples::OneMaxChromosome, 6)
      ...
      def demonstrate_fitness_landscapes
        ...
        result = ga.run(Examples::RastriginChromosome, 5)
        ga.plot_fitness
      end
      ...
      def demonstrate_visualization_tools
        ...
        result = ga.run(Examples::SphereChromosome, 5)
...
class Symbol
  def humanize
    self.to_s.gsub('_', ' ').split.map(&:capitalize).join(' ')
  end
end