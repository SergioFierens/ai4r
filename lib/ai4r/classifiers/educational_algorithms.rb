  ...
  module Classifiers
    ...
    class EducationalID3 < ID3
      ...
      def build_with_steps(data_set)
        ...
        @tree = build_tree_with_steps(data_set.data_items, [], 2) { |step_info| yield(step_info) }
      ...
      def build_tree_with_steps(data_examples, used_attributes, step_number, &block)
        ...
        if data_examples.nil? || data_examples.empty?
          return ErrorNode.new
        end
        ...
        best_attribute_index = find_best_attribute_with_steps(data_examples, domain, used_attributes, step_number, &block)
        ...
        child_nodes = split_data_examples.map.with_index do |subset, index|
          if subset && !subset.empty?
            build_tree_with_steps(subset, used_attributes + [best_attribute_index], step_number + 10 + index * 10, &block)
          else
            most_common_class = most_freq(data_examples, domain)
      ...
      def find_best_attribute_with_steps(data_examples, domain, used_attributes, step_number, &block)
        best_attribute = nil
        ...
        domain[0...-1].each_with_index do |attribute_domain, index|
      ...
      def calculate_information_gain(data_examples, freq_grid, current_entropy)
    ...
    class EducationalNaiveBayes < NaiveBayes
      ...
      def calculate_probabilities_with_steps(&block)
        ...
        @data_items.each do |item|
          @data_labels.each_with_index do |label, dl_index|
    ...
    class EducationalMultilayerPerceptron < MultilayerPerceptron
      ...
      def build_with_steps(data_set)
        ...
        @training_iterations.times do |epoch|
          ...
          data_set.data_items.each_with_index do |data_item, example_index|
    ...
    class EducationalOneR < OneR
      ...
      def build_with_steps(data_set)
        ...
        @data_set.data_labels[0...-1].each_with_index do |attribute_name, attr_index|
          ...
          if @configuration.verbose
            puts "    Attribute '#{attribute_name}': Error rate = #{error_rate.round(4)}"
          end
