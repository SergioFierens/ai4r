      ...
      class DatasetGenerator
        def self.generate_simple_binary_dataset
          ...
          data_items = [
            ...
            %w[sunny hot high no],
            %w[sunny hot high no],
            %w[overcast hot high yes],
            %w[rainy mild high yes],
            %w[rainy cool normal yes],
            %w[rainy cool normal no],
            %w[overcast cool normal yes],
            %w[sunny mild high no],
            %w[sunny cool normal yes],
            %w[rainy mild normal yes],
            %w[sunny mild normal yes],
            %w[overcast mild high yes],
            %w[overcast hot normal yes],
            %w[rainy mild high no]
          ]

          data_labels = %w[weather temperature humidity play_tennis]
        ...
        def self.generate_multiclass_dataset
          ...
          data_items = [
            ...
            %w[short wide short narrow setosa],
            %w[short wide short narrow setosa],
            %w[short medium short narrow setosa],
            %w[medium wide short narrow setosa],
            %w[short narrow short narrow setosa],
            %w[medium medium medium medium versicolor],
            %w[long narrow medium medium versicolor],
            %w[medium medium medium wide versicolor],
            %w[medium narrow medium narrow versicolor],
            %w[medium narrow medium medium versicolor],
            %w[long narrow long wide virginica],
            %w[medium narrow long wide virginica],
            %w[long narrow long medium virginica],
            %w[long medium long wide virginica],
            %w[medium wide long wide virginica]
          ]
        ...
        def self.generate_marketing_dataset
          ...
          data_items = [
            %w[young student single fair no],
            %w[young student single excellent no],
            %w[middle student single fair yes],
            %w[senior employee single fair yes],
            %w[senior employee single excellent no],
            %w[senior employee married excellent no],
            %w[middle employee married excellent yes],
            %w[young student single fair no],
            %w[young employee married fair yes],
            %w[senior employee married fair yes],
            %w[young employee married excellent yes],
            %w[middle student married excellent yes],
            %w[middle employee single fair yes],
            %w[senior employee single excellent no]
          ]

          data_labels = %w[age job marital credit_rating buys_computer]
          Ai4r::Data::DataSet.new(data_labels: data_labels, data_items: data_items)
        end
        ...
        def self.generate_medical_dataset
          ...
          data_items = [
            %w[high yes normal no heart_disease],
            %w[normal no high yes healthy],
            %w[high yes high no heart_disease],
            %w[normal no normal no healthy],
            %w[high no high yes heart_disease],
            %w[normal yes normal no healthy],
            %w[high yes high yes heart_disease],
            %w[normal no normal yes healthy],
            %w[high no normal no heart_disease],
            %w[normal yes high no diabetes],
            %w[high yes normal yes heart_disease],
            %w[normal no high no diabetes],
            %w[high no high no heart_disease],
            %w[normal yes high yes diabetes]
          ]

          data_labels = %w[blood_pressure chest_pain cholesterol family_history diagnosis]
          Ai4r::Data::DataSet.new(data_labels: data_labels, data_items: data_items)
        end
        ...
        def self.generate_numeric_dataset
          ...
          data_items = [
            ['male', 25, 'bachelor', 50_000, 'approved'],
            ['female', 35, 'master', 75_000, 'approved'],
            ['male', 28, 'bachelor', 45_000, 'rejected'],
            ['female', 42, 'phd', 95_000, 'approved'],
            ['male', 30, 'high_school', 35_000, 'rejected'],
            ['female', 26, 'bachelor', 52_000, 'approved'],
            ['male', 38, 'master', 68_000, 'approved'],
            ['female', 33, 'bachelor', 48_000, 'rejected'],
            ['male', 45, 'phd', 120_000, 'approved'],
            ['female', 29, 'high_school', 32_000, 'rejected'],
            ['male', 31, 'master', 71_000, 'approved'],
            ['female', 27, 'bachelor', 46_000, 'rejected'],
            ['male', 39, 'phd', 98_000, 'approved'],
            ['female', 34, 'master', 62_000, 'approved']
          ]

          data_labels = %w[gender age education income loan_status]
          Ai4r::Data::DataSet.new(data_labels: data_labels, data_items: data_items)
        end
        ...
        def self.generate_imbalanced_dataset
          ...
          data_items = [
            %w[low small new normal],
            %w[low small new normal],
            %w[low small old normal],
            %w[low medium new normal],
            %w[low medium old normal],
            %w[medium small new normal],
            %w[medium small old normal],
            %w[medium medium new normal],
            %w[medium medium old normal],
            %w[medium large new normal],
            %w[high small new normal],
            %w[high medium new normal],
            %w[high large old fraud], # Only 2 fraud cases
            %w[high large new fraud]
          ]

          data_labels = %w[risk_score transaction_amount account_age transaction_type]
          Ai4r::Data::DataSet.new(data_labels: data_labels, data_items: data_items)
        end

        def self.split_dataset(dataset, train_ratio = 0.7, _test_ratio = 0.3)
      ...
      class << self
        ...
        def run_algorithm_comparison
          ...
          results.each_value do |result|
            ...
            puts format('%-22s | %8.4f | %11.4f', result[:name], evaluation[:accuracy], avg_f1)
          end
        ...
        def run_cross_validation_example
          ...
          algorithms = %i[id3 naive_bayes one_r]
        ...
        def run_feature_importance_analysis
          ...
          test_case = %w[young student single fair]
        ...
        def run_model_interpretability_example
          ...
          test_case = %w[sunny hot high]

          models.each do |algorithm_type, algorithm_name|
            puts "\n--- #{algorithm_name} ---"
            classifiers[algorithm_type].eval(test_case)
          end
        ...
        def create_custom_classification_example(dataset_type, algorithm_type, parameters = {})
          ...
          classifier.enable_step_mode if parameters[:step_mode]

          classifier.enable_visualization if parameters[:visualization]
