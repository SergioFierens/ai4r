...
module Ai4r
  module Classifiers
    ...
    class SupportVectorMachine < Classifier
      ...
      attr_reader :support_vectors, :alphas, :bias, :kernel_type, :kernel_params
      attr_reader :c_parameter, :training_errors, :support_vector_indices
      attr_reader :educational_mode, :training_steps, :decision_function_values
      ...
      def eval(data_item)
        ...
        if @educational_mode && @verbose
          explain_prediction(data_item, decision_value, prediction)
        end
      ...
      def simple_smo_algorithm
        ...
        max_iter.times do |iteration|
          alpha_changed = 0
          
          @training_data.length.times do |i|
            error_i = decision_function_for_training(i) - @labels[i]
            
            if (@labels[i] * error_i < -@tolerance && @alphas[i] < @c_parameter) ||
      ...
      def decision_function_for_training(i)
        ...
        @training_data.length.times do |j|
          if @alphas[j] > 0
            kernel_val = @gram_matrix ? @gram_matrix[j][i] : 
      ...
      def select_second_alpha(i, error_i)
      ...
      def update_bias(i, j, error_i, error_j, alpha_i_old, alpha_j_old, k_ii, k_jj, k_ij)
        ...
        if @alphas[i] > 0 && @alphas[i] < @c_parameter
          @bias = b1
