...
module Ai4r
  module Classifiers
    ...
    class ClassifierEvaluationSuite
      ...
      def comprehensive_evaluation(classifier, test_set, _options = {})
        ...
        explain_evaluation_results(results) if @educational_mode
      ...
      def compare_classifiers(classifiers, test_set, options = {})
        ...
        classifiers.each_with_index do |classifier, _idx|
        ...
        explain_classifier_comparison(comparison_results) if @educational_mode
      ...
      def cross_validation_evaluation(classifier_class, data_set, k_folds = 5, options = {})
        ...
        folds.each_with_index do |fold, idx|
          ...
          classifier.configure(options[:classifier_config]) if options[:classifier_config]
        ...
        explain_cross_validation_results(cv_summary, fold_results) if @educational_mode
      ...
      def learning_curve_analysis(classifier_class, data_set, options = {})
        ...
        explain_learning_curves(results) if @educational_mode

        results
      end

      # Feature importance analysis across classifiers
      def feature_importance_analysis(classifiers, data_set, _options = {})
        ...
        explain_feature_importance(importance_results, aggregated) if @educational_mode
      ...
      def calculate_class_metrics(predictions, actual_labels)
        ...
        classes.each do |class_name|
          tp = predictions.zip(actual_labels).count do |pred, actual|
            pred == class_name && actual == class_name
          end
          fp = predictions.zip(actual_labels).count do |pred, actual|
            pred == class_name && actual != class_name
          end
          fn = predictions.zip(actual_labels).count do |pred, actual|
            pred != class_name && actual == class_name
          end
          tn = predictions.zip(actual_labels).count do |pred, actual|
            pred != class_name && actual != class_name
          end
      ...
      def calculate_binary_roc_from_arrays(binary_labels, probabilities)
        ...
        sorted_indices.each_with_index do |idx, _rank|
      ...
      def calculate_reliability_diagram(binary_labels, probabilities, num_bins = 10)
        ...
        bins.map_with_index do |bin, idx|
          if bin[:count] > 0
      ...
      def create_stratified_folds(data_set, k_folds)
        ...
        class_groups.each_value do |items|
      ...
      def explain_evaluation_results(results)
        ...
        results[:class_metrics].each do |class_name, metrics|
          next if %i[macro_avg micro_avg].include?(class_name)
        ...
        return unless results[:statistical_measures][:cohens_kappa]
      ...
      def perform_statistical_comparison(_classifiers, _test_set)
      ...
      def rank_classifiers(results)
        ...
        metrics_to_rank = %i[accuracy macro_f1 micro_f1]
        ...
        metrics_to_rank.each do |metric|
          ...
          rankings[metric] = scores.sort_by { |_name, score| -score }.to_h
        end

        rankings
      end

      def generate_comparison_summary(_results, ranking)
        summary = {}
      ...
      def calculate_permutation_importance(_classifier, data_set)
      ...
      def aggregate_feature_importance(_importance_results, _data_set)
        # Placeholder for aggregating feature importance across classifiers
        {}
      end

      def explain_feature_importance(_individual_results, _aggregated)
        puts "\n=== Feature Importance Analysis ==="
