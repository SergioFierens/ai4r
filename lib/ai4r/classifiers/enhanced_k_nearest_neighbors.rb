...
module Ai4r
  module Classifiers
    ...
    class EnhancedKNearestNeighbors < Classifier
      ...
      attr_reader :k, :distance_metric, :weighting_scheme, :training_data, :training_labels
      attr_reader :educational_mode, :prediction_explanations, :distance_calculations
      attr_reader :feature_weights, :normalization_params
      ...
      def build(data_set)
        ...
        if @normalize_features
          normalize_training_features
        end
        
        # Build spatial index if requested
        if @use_kd_tree && @training_data.first.length <= 10
          build_kd_tree
        end
      ...
      def eval(data_item)
        ...
        if @educational_mode && @explain_predictions
          explain_prediction(data_item, query_point, neighbors, prediction)
        end
      ...
      def find_optimal_k(k_range = (1..20), cv_folds = 5)
        ...
        # Find best k
        best_k = results.max_by { |k, result| result[:accuracy] }[0]
        
        if @educational_mode
          explain_k_selection(results, best_k)
        end
      ...
      def analyze_curse_of_dimensionality
        ...
        if @educational_mode
          explain_curse_of_dimensionality(analysis)
        end
      ...
      def compare_distance_metrics(test_point = nil)
        ...
        metrics.each do |metric|
          ...
          results[metric] = {
            ...
            avg_distance: distances.map { |item| item[:distance] }.sum / distances.length
          }
      ...
      def set_distance_metric(metric, params = {})
        valid_metrics = [:euclidean, :manhattan, :chebyshev, :minkowski, :cosine, :hamming]
        
        unless valid_metrics.include?(metric)
          raise ArgumentError, "Unsupported distance metric: #{metric}"
        end
        
        @distance_metric = metric
        
        if metric == :minkowski
          @minkowski_p = params[:p] || params[:minkowski_p] || 2
        end
      end
      
      def set_weighting_scheme(scheme)
        valid_schemes = [:uniform, :distance, :inverse_distance]
        
        unless valid_schemes.include?(scheme)
          raise ArgumentError, "Unsupported weighting scheme: #{scheme}"
        end
      ...
      def prepare_training_data
        ...
        @training_labels = @data_set.data_items.map { |item| item.last }
      ...
      def normalize_training_features
        ...
        @training_data.each do |example|
          example.each_with_index do |feature, idx|
            ...
            if std > 0
              example[idx] = (feature - @normalization_params[:means][idx]) / std
            else
              example[idx] = 0.0
            end
          end
      ...
      def preprocess_query_point(data_item)
        ...
        if @normalize_features && !@normalization_params[:means].empty?
          query_point.each_with_index do |feature, idx|
            std = @normalization_params[:stds][idx]
            if std > 0
              query_point[idx] = (feature - @normalization_params[:means][idx]) / std
            else
              query_point[idx] = 0.0
            end
          end
      ...
      def predict_from_neighbors(neighbors, query_point)
        class_votes = calculate_class_votes(neighbors)
        
        # Return class with highest vote
        class_votes.max_by { |label, weight| weight }[0]
      end
      ...
      def explain_prediction(original_point, query_point, neighbors, prediction)
        ...
        if @normalize_features
          puts "Normalized query: #{query_point.map { |x| x.round(4) }.inspect}"
        end
        ...
        neighbors.each_with_index do |neighbor, idx|
          weight = calculate_neighbor_weight(neighbor)
          puts sprintf("  %d. Distance: %8.4f, Label: %s, Weight: %6.3f", 
