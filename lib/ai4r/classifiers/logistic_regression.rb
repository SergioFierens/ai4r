...
module Ai4r
  module Classifiers
    ...
    class LogisticRegression < Classifier
      ...
      attr_reader :weights, :bias, :learning_rate, :regularization_type, :lambda
      attr_reader :cost_history, :iteration_count, :feature_importance
      attr_reader :educational_mode, :training_steps, :class_labels
      ...
      def eval(data_item)
        ...
        if @classification_type == :binary
          ...
          if @educational_mode && @verbose
            explain_binary_prediction(data_item, processed_item, probability, prediction)
          end
          ...
          if @educational_mode && @verbose
            explain_multinomial_prediction(data_item, probabilities, prediction)
          end
      ...
      def plot_cost_history
        ...
        @cost_history.each_with_index do |cost, idx|
          ...
          puts sprintf("%9d | %9.6f | %+9.6f", idx + 1, cost, change)
        end
      ...
      def evaluate_performance(test_set)
        ...
        if @educational_mode
          explain_performance_metrics(performance)
        end
      ...
      def set_regularization(reg_type, lambda_value)
        ...
        unless [:none, :l1, :l2, :elastic_net].include?(reg_type)
      ...
      def prepare_training_data(data_set)
        ...
        raw_labels = data_set.data_items.map { |item| item.last }
      ...
      def update_weights(gradients)
        if @classification_type == :binary
          ...
          @weights.each_with_index do |class_weights, class_idx|
            class_weights.each_with_index do |weight, feat_idx|
      ...
      def add_regularization_gradients(weight_gradients)
        ...
        case @regularization_type
        when :l1
          weight_gradients.each_with_index do |grad, idx|
      ...
      def add_regularization_gradients_multinomial(weight_gradients)
        ...
        when :l1
          weight_gradients.each_with_index do |class_gradients, class_idx|
            class_gradients.each_with_index do |grad, feat_idx|
