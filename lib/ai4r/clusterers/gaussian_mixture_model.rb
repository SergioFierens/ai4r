...
module Ai4r
  module Clusterers
    ...
    class GaussianMixtureModel < Clusterer
      ...
      attr_reader :n_components, :means, :covariances, :weights
      attr_reader :responsibilities, :log_likelihood_history, :iterations
      attr_reader :data_set, :clusters, :soft_assignments
      ...
      def visualize_model
        ...
        if @log_likelihood_history.length > 1
          ...
          @log_likelihood_history.each_with_index do |ll, iter|
            if iter == 0
              puts sprintf("%9d | %14.4f | %s", iter, ll, "Initial")
            else
              improvement = ll - @log_likelihood_history[iter - 1]
              puts sprintf("%9d | %14.4f | %10.6f", iter, ll, improvement)
            end
          end
        end
      ...
      def all_numeric?
        @data_set.data_items.all? do |item|
          item.all? { |attr| attr.is_a?(Numeric) }
        end
      end
      ...
      def calculate_single_point_responsibilities(point)
        unnormalized = @n_components.times.map do |k|
      ...
      def visualize_2d_gaussian_mixture
        ...
        @means.each_with_index do |mean, k|
          x, y = mean[0], mean[1]
          plot_x = ((x - x_min) / (x_max - x_min) * (plot_width - 1)).to_i
          plot_y = ((y - y_min) / (y_max - y_min) * (plot_height - 1)).to_i
    ...
    class GMMParameterHelper
      ...
      def self.suggest_components(data_set, max_components = 10)
        ...
        best_ll = results.max_by { |r| r[:log_likelihood] }
        ...
        results.each do |result|
          ...
          puts sprintf("%2d | %14.2f | %s%9.2f | %s%9.2f | %10d",
      ...
      def self.compare_covariance_types(data_set, k = 3)
        ...
        covariance_types = ['spherical', 'diagonal', 'full']
        results = {}
        ...
        results.each do |cov_type, result|
          puts sprintf("%-15s | %14.2f | %10d | %10d",
      ...
      def self.generate_k_recommendations(results)
        ...
        if results.length >= 3
          ...
          if improvements.length >= 2
            ...
            if diminish_point
              recommendations << "Elbow method suggests K = #{diminish_point + 1}"
            end
          end
        end
