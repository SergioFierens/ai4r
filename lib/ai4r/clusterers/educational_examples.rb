  ...
  module Clusterers
    ...
    class EducationalExamples
        ...
        datasets_info.each do |dataset_info|
          ...
          algorithms.each do |algorithm|
            ...
            begin
              ...
              puts "  Noise points: #{result[:stats][:num_noise_points]}" if result[:stats][:num_noise_points]
      ...
      def self.generate_customer_segmentation_data(n_customers)
        data_items = []
        labels = %w[age income purchases_per_month website_hours]

        # Define customer segments
        segments = [
          { age: [22, 35], income: [25_000, 45_000], purchases: [2, 8], hours: [5, 15] },   # Young shoppers
          { age: [35, 50], income: [45_000, 75_000], purchases: [3, 12], hours: [3, 10] },  # Middle-aged
          { age: [50, 70], income: [60_000, 100_000], purchases: [1, 6], hours: [1, 5] },   # Older, higher income
          { age: [25, 40], income: [75_000, 120_000], purchases: [8, 20], hours: [10, 25] } # Premium customers
        ]
      ...
      def self.generate_network_traffic_data(n_samples)
        data_items = []
        labels = %w[packet_size frequency duration]
      ...
      def self.generate_medical_diagnosis_data(n_patients)
        data_items = []
        labels = %w[temperature blood_pressure heart_rate symptom_severity]
      ...
      def self.generate_mixed_scale_data(n_points)
        data_items = []
        labels = %w[age income purchases]

        n_points.times do
          age = rand(20..80)
          income = rand(20_000..100_000)
          purchases = rand(1..50)
      ...
      def self.analyze_customer_segments(result, _data_set)
