...
module Ai4r
  module Clusterers
    ...
    class ClusteringConfiguration
      attr_accessor :distance_function, :max_iterations, :tolerance, :verbose
      attr_accessor :initialization_strategy, :linkage_type
    ...
    class ClusteringMonitor
      ...
      def summary
        ...
        if @convergence_history.any?
          summary[:convergence_rate] = calculate_convergence_rate
        end
      ...
      def plot_convergence
        ...
        @convergence_history.each do |point|
          ...
          bar = "█" * bar_length
          puts sprintf("%9d | %s %.6f", point[:iteration], bar, point[:change])
        end
      ...
      def compare
        ...
        quality1.each do |metric, value1|
          ...
          if value1.is_a?(Numeric) && value2.is_a?(Numeric)
            ...
            puts sprintf("%-25s: %.4f vs %.4f (%s)", metric, value1, value2, better)
          else
            puts sprintf("%-25s: %s vs %s", metric, value1, value2)
          end
        end
