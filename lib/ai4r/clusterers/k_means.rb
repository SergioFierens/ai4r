...
module Ai4r
  module Clusterers
    ...
    class KMeans < Clusterer
      attr_reader :data_set, :number_of_clusters, :clusters, :centroids, :iterations
      ...
      def build(data_set, number_of_clusters)
        ...
        unless @centroid_indices.empty? || @centroid_indices.length == @number_of_clusters
          raise ArgumentError, 'Length of centroid indices array differs from the specified number of clusters'
        end
        unless %w[eliminate terminate random outlier].include?(@on_empty)
          raise ArgumentError, 'Invalid value for on_empty'
        end
        ...
        when 'indices' # for initial assignment only (with the :centroid_indices option)
          @centroid_indices.each do |index|
            unless (index.is_a? Integer) && index >= 0 && index < @data_set.data_items.length
              raise ArgumentError, "Invalid centroid index #{index}"
            end
