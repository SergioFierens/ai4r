...
module Ai4r
  module Clusterers
    ...
    class DBSCAN < Clusterer
      ...
      attr_reader :eps, :min_pts, :clusters, :noise_points, :core_points
      attr_reader :data_set, :point_types, :cluster_assignments
      ...
      def identify_border_points
        @data_set.data_items.each_with_index do |point, point_index|
          ...
          if @point_types[point_index] == NOISE
            @noise_points << point
          end
        end
      end
      ...
      def finalize_results
        ...
        @clusters = @clusters.select { |cluster| !cluster.data_items.empty? }
      end
