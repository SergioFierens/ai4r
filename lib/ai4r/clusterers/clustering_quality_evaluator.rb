  ...
  module Clusterers
    ...
    class ClusteringQualityEvaluator
      ...
      def evaluate_all_metrics
        ...
        metrics[:variance_ratio] = if metrics[:between_cluster_sum_of_squares] > 0
                                     metrics[:between_cluster_sum_of_squares] / metrics[:total_sum_of_squares]
                                   else
                                     0
                                   end
      ...
      def self.compare_clusterings(evaluations)
        ...
        metrics = evaluations.first[:metrics].keys.reject { |k| k == :cluster_statistics }
      ...
      def explain_silhouette_score(score)
        ...
        explanation += case score
                       when 0.7..1.0
                         'Excellent clustering (0.7-1.0) - strong, well-separated clusters'
                       when 0.5..0.7
                         'Good clustering (0.5-0.7) - reasonable structure with some overlap'
                       when 0.2..0.5
                         'Weak clustering (0.2-0.5) - artificial clustering or poor fit'
                       when -1.0..0.2
                         'Poor clustering (-1.0-0.2) - points may be in wrong clusters'
                       else
                         'Invalid score range'
                       end
      ...
      def explain_davies_bouldin(db_index)
        ...
        explanation += case db_index
                       when 0.0..0.5
                         'Excellent clustering (0.0-0.5) - compact and well-separated clusters'
                       when 0.5..1.0
                         'Good clustering (0.5-1.0) - reasonable cluster quality'
                       when 1.0..2.0
                         'Moderate clustering (1.0-2.0) - some overlap or poor separation'
                       else
                         'Poor clustering (>2.0) - clusters poorly defined'
                       end
      ...
      def explain_calinski_harabasz(ch_index)
        ...
        explanation += if ch_index > 100
                         'High score suggests well-defined clusters with good separation.'
                       elsif ch_index > 50
                         'Moderate score - reasonable clustering structure.'
                       else
                         'Low score - weak clustering structure or poor parameter choice.'
                       end
      ...
      def explain_dunn_index(dunn_index)
        ...
        explanation += case dunn_index
                       when 0.5..Float::INFINITY
                         'Excellent clustering (>0.5) - well-separated, compact clusters'
                       when 0.2..0.5
                         'Good clustering (0.2-0.5) - reasonable separation'
                       when 0.1..0.2
                         'Moderate clustering (0.1-0.2) - some overlap'
                       else
                         'Poor clustering (<0.1) - significant overlap or poor structure'
                       end
      ...
      def self.generate_comparison_insights(evaluations)
        ...
        metrics = %i[silhouette_score variance_ratio calinski_harabasz_index]
