...
module Ai4r
  module Som
    ...
    class SomConfiguration
      attr_accessor :epochs, :initial_learning_rate, :initial_radius, :layer_type, :verbose, :neighborhood_function,
                    :learning_rate_decay, :radius_decay
    ...
    class SomMonitor
      attr_reader :start_time, :training_time, :quantization_errors, :epoch_data, :final_quantization_error,
                  :epochs_completed
    ...
    class EducationalSomCore < Som
      ...
      def train_with_steps(training_data)
        ...
        @layer.epochs.times do |epoch|
          ...
          # Record epoch data
          @monitor.record_epoch(epoch, avg_quantization_error, {
      ...
      def get_all_node_weights
        ...
        (0...@number_of_nodes).each do |x|
          weights_row = (0...@number_of_nodes).map do |y|
            get_node(x, y).weights.dup
          end
        ...
        training_data.each_with_index do |input1, i|
          training_data.each_with_index do |input2, j|
            ...
            training_data.each_with_index do |input3, k|
              ...
              preservation_score += 1 if (input_distance < input_distance_3) == (map_distance < map_distance_3)
              total_comparisons += 1
            end
          end
        end
    ...
    class SomQualityAnalyzer
      ...
      def analyze
        ...
        # Map quality summary
        quality_score = calculate_overall_quality(quantization_error, topographic_error, neighborhood_preservation,
                                                  utilization)
      ...
      def calculate_topographic_error
        ...
        @training_data.each do |input|
          ...
          topographic_errors += 1 unless x_diff <= 1 && y_diff <= 1 && (x_diff + y_diff > 0)
        end
    ...
    class SomTopologyAnalyzer
        ...
        (0...@som.number_of_nodes).each do |x|
          (0...@som.number_of_nodes).each do |y|
            ...
            [[x + 1, y], [x - 1, y], [x, y + 1], [x, y - 1]].each do |nx, ny|
              next unless nx >= 0 && nx < @som.number_of_nodes && ny >= 0 && ny < @som.number_of_nodes
      ...
      def suggest_improvements
        ...
        puts '  • Consider increasing training epochs for better convergence' if @configuration.epochs < 100
    ...
    class SomVisualizer
      ...
      def visualize_map_structure
        ...
        (0...@map_info[:map_size]).each do |_x|
          print '  '
          (0...@map_info[:map_size]).each do |_y|
            print '◯ '
          end
          puts
        end
      ...
      def visualize_2d_weights
        ...
        (0...@map_info[:map_size]).each do |x|
          (0...@map_info[:map_size]).each do |y|
            ...
            print format('(%5.2f,%5.2f) ', weights[0], weights[1])
          end
          puts
        end
      end
      ...
      def visualize_3d_weights
        ...
        (0...@map_info[:map_size]).each do |x|
          (0...@map_info[:map_size]).each do |y|
            weights = @som.get_node(x, y).weights
            print format('(%4.2f,%4.2f,%4.2f) ', weights[0], weights[1], weights[2])
          end
          puts
        end
      end
      ...
      def visualize_training_progress
        ...
        if @training_history.length > 10
          ...
          sample_epochs.each do |index|
            ...
            next unless epoch_data

            puts format('%5d | %11.6f | %10.4f | %6.2f',
                        epoch_data[:epoch],
    ...
    class SomExporter
      ...
      def export_csv(filename)
        ...
        CSV.open(filename, 'w') do |csv|
          # Header
          headers = %w[x y] + (0...@map_info[:input_dimension]).map { |i| "weight_#{i}" }
          csv << headers
